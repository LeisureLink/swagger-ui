...
apisSorter | Apply a sort to the API/tags list. It can be 'alpha' (sort by name) or an ordered array of tags (any tags not explicitly listed in the array will appear after the explicit ordered list in the order they are returned from server), or a function (see Array.prototype.sort() to know how sort function works). Default is the order returned by the server unchanged.
